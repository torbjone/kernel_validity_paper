...
# Modified from brunel_delta_nest.py!
...
def run_brunel_network(g=5.0, eta=2.0, J=0.1, order=2500, N_rec=100, basename="test"):
    savefolder = "brunel_sim_delta"
    os.makedirs(savefolder, exist_ok=True)
    name = "{:s}_g={:1.1f}_eta={:1.1f}_J={:1.1f}_order={:d}_Nrec={:d}".format(basename, g, eta, J, order, N_rec)
    ...
    simtime = 20200.0  # Simulation time in ms
    ...
    espikes.set(label="brunel_ex_{}".format(name), record_to="memory")
    ispikes.set(label="brunel_in_{}".format(name), record_to="memory")
    ...
    plot_raster(espikes, ispikes, name)
...
def sort_spiketrains(spikes):
    ...
    ids = spikes.events["senders"]

    spiketrains = []
    for gid in range(np.min(ids), np.max(ids + 1)):
        spiketimes = times[np.where(ids == gid)[0]]
        spiketrains.append(spiketimes)
    return spiketrains


def plot_raster(espikes, ispikes, name):
    times_ex = espikes.events["times"]
    ids_ex = espikes.events["senders"]

    times_in = ispikes.events["times"]
    ids_in = ispikes.events["senders"]
    plt.close("all")
    fig = plt.figure(figsize=[10, 10])
    ax1 = fig.add_subplot(111, xlim=[1000, 1400])
...
if __name__ == '__main__':
    ...
    #run_brunel_network(g=3, eta=2.0, J=0.1, order=2500, N_rec=1000, basename="SR")
    #run_brunel_network(g=6.0, eta=4.0, J=0.1, order=2500, N_rec=1000, basename="SI_fast")
    run_brunel_network(g=5., eta=2.0, J=0.1, order=2500, N_rec=1000, basename="AI_slow")
    run_brunel_network(g=4.5, eta=0.9, J=0.1, order=2500, N_rec=1000, basename="SI_slow")
    # run_brunel_network(g=5.5, eta=2.5, J=0.1, order=2500, N_rec=1000, basename="AI_fast")
    # run_brunel_network(g=4.5, eta=2.0, J=0.1, order=2500, N_rec=1000, basename="AI_slow")
