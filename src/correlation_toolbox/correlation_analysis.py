...
def powerspec(data, tbin, Df=None, units=False, N=None):
    ...
    **Examples**:
    ...
    # OLD: POW *= 1. / T * 1e3  # normalization, power independent of T
    POW /= (T/tbin)
    return freq, POW
...
def autocorrfunc_time(spike_trains, tau_max, bin_size, T):
    '''
    Calculate autocorrelation function(s) for given spike trains.

    **Args**:
       spike_trains: 1 dim numpy.ndarray
    ...
    **Return**:
    ...
    inds = np.where(spike_trains>tau_max)
    trimmed_spikes = spike_trains[inds]
    inds = np.where(trimmed_spikes<T-tau_max)
    trimmed_spikes = trimmed_spikes[inds]
    # loop of spikes
    for spike in trimmed_spikes:
        # get difference time difference between this spike and all other spikes
        diff = spike_trains-spike
    ...
    auto /= (T-2*tau_max)*bin_size*1e-6
    return time, auto

def autocorrfunc_time_backup(spike_trains, tau_max, bin_size, T, units=False):
    '''
    Calculate autocorrelation function(s) for given spike trains.
...
def create_spike_counts(spikes, binsize, T):
    '''
    Create spike counts from spike trains.

    Parameters
    ----------

    spikes:  list(list(float)
             List of spike times (s) for each of the N realisations

    binsize: float
             Length of each time bin (s)
    ...
    spike_counts: numpy.ndarray
                  NxD array of spike counts (number of spikes per time bin)
                  for the N spike trains in `spikes` and a total number D=ceil(T/binsize) of time bins.

    bins:         numpy.ndarray
                  Array of D time bins (s)
    '''

    N = len(spikes)  ## ensemble size
    bins = np.arange(0, T + binsize, binsize)  ## discretized time (bins)
    spike_counts = np.zeros((N, len(bins) - 1))
    for cs in range(N):
        spike_counts[cs, :] = np.histogram(spikes[cs], bins)[0]
    return spike_counts, bins[:-1]


def spike_count_correlation_coefficient(spikes, binsize, T):
    '''
    Calculates the population averaged spike-count correlation coefficient for
    an ensemble of N spikes trains at a given binsize.

    Note: Here, we calculate the population averaged spike-count covariance c from

    - the variance a_z of the compound signal z = sum_i s_i, and
    - the averaged single-spike-count variance a

    by solving

       a_z = N*a + N*(N-1)c

    for c (see eq. (2.4.4) in https://fz-juelich.sciebo.de/s/jRiKiGpTxLSrvQS).

    Parameters
    ----------

    spikes:  list(list(float)
             List of spike times (s) for each of the N spike trains
    ...
    N = len(spikes)  ## population size

    ## generate spike counts from spike data
    spike_counts, bins = create_spike_counts(spikes, binsize, T)

    z = np.sum(spike_counts, 0)  ## compound (sum) spike count

    a_z = np.var(z)  ## variance of compound spike count

    a = 0
    for i in range(N):
        a += np.var(spike_counts[i, :])  ## single-spike-count variances (cumulative pop. averaging)
    a /= N  ## population averaged spike-count variance

    c = (a_z - N * a) / (N * (N - 1))  ## population averaged spike-count covariance

    r = c / a  ## population averaged spike-count correlation coefficient
